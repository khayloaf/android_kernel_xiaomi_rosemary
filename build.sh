...
function KERNEL_RESULT() {
	...
	# Create anykernel
	rm -rf anykernel
	git clone https://github.com/khayloaf/AK3-Rosemary anykernel

	# Copying image
	cp ${KERNEL_PATH}/Image.gz-dtb anykernel/kernels/
	cp ${KERNEL_PATH}/dts/mediatek/*.dtbo anykernel/kernels/dtbo.img
